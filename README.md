...
```
pixel_raw [7:0]
      │  normalise: pixel_raw[7:5] → Q4.3
      ▼
┌──────────┐        ┌──────────┐  ┌──────────┐        ┌──────────┐  ┌──────────┐
│  Conv1   │─valid─▶│  Pool1   │  │  Conv2   │─valid─▶│  Pool2   │  │    FC    │
│ 3×3, ×8  │◀─stall─│ 26²→13² │──▶│ 3×3, ×16│◀─stall─│ 11²→5²  │──▶│ 400→10  │──▶ digit_out
│ 28²→26² │         └──────────┘  │ 13²→11² │         └──────────┘  │ argmax  │
└──────────┘                      └──────────┘                       └──────────┘
```

| Layer | Operation | Input | Output | DSP48 Slices |
|-------|-----------|-------|--------|--------------|
| Conv1 | 3×3 conv + ReLU | 28×28×1 | 26×26×8 | 72 |
| Pool1 | 2×2 max pool | 26×26×8 | 13×13×8 | 0 |
| Conv2 | 3×3 conv + ReLU | 13×13×8 | 11×11×16 | 144 |
| Pool2 | 2×2 max pool | 11×11×16 | 5×5×16 | 0 |
